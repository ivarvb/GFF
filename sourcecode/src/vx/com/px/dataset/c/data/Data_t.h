...
//local
void vx_densemat_addnewrow(DenseMat_t *dat, char **words, int wcols, int* colsids, int sizecolsids);
...
void vx_densemat_addnewrow(DenseMat_t *dat, char **words, int wcols, int* colsids, int sizecolsids){
    ...
    dat->cols = sizecolsids;
    dat->rows++;
    dat->data = (double*)realloc(dat->data, ((dat->rows)*dat->cols)*sizeof(double));
    int j;
    double v;
    for (j=0; j<dat->cols; ++j){
        v = atof(words[colsids[j]]);
        //words[j+indexuid];
        dat->data[(dat->rows-1)*dat->cols+j] = v;
    }
}
void vx_densemat_addrow(DenseMat_t *dat){
    dat->rows++;
...
DenseMat_t* vx_densemat_from_csv(char filename[],  int* colsids, int sizecolsids){
    ...
    int wcols = vx_densemat_maketokens(line, delim, words);
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        vx_densemat_addnewrow(dat, words, wcols, colsids, sizecolsids);
    }
    wcols = sizetokens;
    fclose(fp);
    if (line)
