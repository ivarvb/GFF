...
#                fileso.append(str(os.path.join(outdir, str(name))))
                ...
                if len(ro)>0:
                    for row in ro:
                        if len(row)>0:
                            ...
                            """ 
                            print("rowrowrowrowrowrowrowrowrowrowrowrowrowrow",row)
                            read = {}
    ...
    def getStatus(app):
        ...
        for r in re:
            if not "statusopt" in r or  not "statusval" in r:
                DBX.update(DBS.DBGFF, "data", {"_id": idin},
    ...
    def savegraph(app,p):
        ...
        #print("1111111111111X")
        filename = Settings.DATA_PATH+str(idin)+"/feature.obj"
        with open(filename, mode="w", encoding='utf-8-sig') as fp:
            ujson.dump(p, fp)
        #print("222222222222X")
    ...
    def makeInstancesLabels(app):
        ...
        for r in re:
            ...
            if "configinstance" in r:
                config = r["configinstance"]
            config["idinstanceslabels"] = colid;
...
class DBFile:
    ...
    def openFile(pathf):
        ...
        try:
            ...
            #print("error read file", pathf)
            dfile = {}
        """
        except IOError as e:
            print("erooooooooooooooooooooooooooooooooooooor", e)
            dfile = {}
        """
